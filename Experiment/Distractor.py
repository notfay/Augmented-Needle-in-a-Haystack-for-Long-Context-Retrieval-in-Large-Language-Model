...
class DistractorConfig:
    text: str
    repeat_count: int  # 1, 3, 5, 7
    target_position_percent: float # Calculated dynamically
...
class AugmentedNeedleHaystack:
    ...
    def create_context(self, needle_config: NeedleConfig, distractor_config: DistractorConfig) -> str:
        sentences = self.haystack_sentences.copy()
        
        # --- INSERTION LOGIC ---
        insertions = [] # List of (index, text)

        # 1. Plan Needle Insertion
        needle_idx = self._calculate_index(needle_config.custom_position_percent, len(sentences))
        insertions.append((needle_idx, needle_config.text))

        # 2. Plan Distractor Insertions (Multiple copies based on repeat_count)
        dist_base_pos = distractor_config.target_position_percent
        
        for _ in range(distractor_config.repeat_count):
            # Add random jitter within ±5% range of needle position
            # e.g., needle at 20% -> distractors between 15-25%
            jitter = (random.random() - 0.5) * 0.10  # ±5%
            final_dist_pos = max(0.0, min(1.0, dist_base_pos + jitter))
            
            dist_idx = self._calculate_index(final_dist_pos, len(sentences))
            insertions.append((dist_idx, distractor_config.text))

        # 3. Execute Insertions (Sort by Index Descending to preserve order)
...
class GeminiProvider:
    def __init__(self, model_name: str = "gemini-1.5-flash"):
        ...
        self.config = types.GenerateContentConfig(temperature=0.0, thinking_config=types.ThinkingConfig(thinking_budget=0)) # Deterministic
...
class ExperimentRunner:
    ...
    def run_experiment(self, config: Dict):
        nc = config['needle']
        qc = config['question']
        dc = config['distractor']
        
        print(f"\n--- Running: {config['name']} ---")
        print(f"Pos: {nc.custom_position_percent:.0%} | Distractors: {dc.repeat_count}")

        niah = AugmentedNeedleHaystack(self.haystack_text)
        context = niah.create_context(nc, dc)

        # Prompt designed to test robustness against distraction
...
ANSWER:"""
        ...
        self.results.append({
            ...
            "needle_text": nc.text,
            "distractor_text": dc.text,
            "position": nc.custom_position_percent,
            "distractor_count": dc.repeat_count,
            "response": response,
            "latency": duration,
    ...
    def save_results(self, filename="DistractorFreq_Results.json"):
...
if __name__ == "__main__":
    ...
    # This CSV must contain: needle_id, needle_text, question_text, distractor_text
    INPUT_CSV = "needles_questions_distractors.csv" 
    DELAY_SECONDS = 70
    ...
    for item in all_data:
        ...
        # Distractor base position = needle position
        # The ±5% jitter in create_context will spread them:
        # 0% -> 0-5%, 10% -> 5-15%, 20% -> 15-25%, etc.
        dist_target = pos
        ...
        for count in distractor_counts:
            ...
            dc = DistractorConfig(
                text=item['distractor_text'], # Loaded from CSV
                repeat_count=count,
                target_position_percent=dist_target
            )

            experiments.append({
                "name": f"DistractorTest_ID{n_id}_Pos{pos*100:.0f}%_Count{count}",
                "needle": nc,
                "question": qc,
                "distractor": dc
            })
    ...
    runner = ExperimentRunner(gemini_provider, haystack, DELAY_SECONDS)
